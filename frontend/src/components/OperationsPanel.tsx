...
const CATEGORIES: { key: string; label: string; ops: string[] }[] = [
...
export function OperationsPanel() {
  ...
  return (
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      <h2 style={{ margin: '0 0 16px 0', fontSize: 20, fontWeight: 600, color: '#1a1a2e' }}>Operations</h2>

      {/* Category Tabs */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20, paddingBottom: 12, borderBottom: '1px solid #e8e8e8' }}>
        {CATEGORIES.map(cat => (
          <button
            key={cat.key}
            onClick={() => { setActiveTab(cat.key); setOp(null); setResult(null); setError('') }}
            style={{
              padding: '6px 14px',
              fontSize: 12,
          ...
          >
            {cat.label}
            <span style={{ marginLeft: 5, fontSize: 10, opacity: 0.7 }}>({cat.ops.length})</span>
          </button>
        ))}
      </div>
          ...
          {activeCategory.ops.map(opKey => {
            ...
            return (
              ...
              >
                ...
                <span style={{ fontSize: 12, color: '#333', lineHeight: 1.3 }}>{opDef.label}</span>
              </button>
            )
          })}
        </div>
      ...
      {op && cfg && (
        <div style={{ border: '1px solid #e0e0e0', borderRadius: 10, padding: 20, background: '#fafbfc', marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 600, color: '#1a1a2e' }}>{cfg.label}</span>
            <span style={{ fontSize: 11, color: '#888', marginLeft: 'auto', fontFamily: 'monospace' }}>{cfg.path}</span>
          </div>
          ...
          <div style={{ display: 'grid', gap: 8, maxWidth: 480, marginBottom: 12 }}>
            {cfg.fields.map(f => (
              <input key={f} placeholder={f}
                ...
                value={params[f] || ''} onChange={e => setParams({ ...params, [f]: e.target.value })} />
            ))}
            {formDef ? (
              <>
                <StructuredForm formDef={formDef} values={formVals} onChange={setFormVals} />
                <button type="button"
                  style={{ fontSize: 11, padding: '4px 10px', background: '#eee', border: '1px solid #ddd', borderRadius: 4, width: 'fit-content', cursor: 'pointer' }}
                  onClick={() => setShowJson(s => !s)}>{showJson ? 'Hide' : 'Preview'} JSON</button>
                {showJson && builtBody && (
                  <pre style={{ fontSize: 11, background: '#f0f0f0', padding: 10, borderRadius: 6, maxHeight: 200, overflow: 'auto', border: '1px solid #e0e0e0' }}>
                    {JSON.stringify(builtBody, null, 2)}
                  </pre>
                )}
              </>
            ) : (cfg.method === 'POST' || cfg.method === 'PUT' || cfg.method === 'PATCH') && (
              <textarea placeholder="JSON body" rows={6}
                style={{ fontFamily: 'monospace', fontSize: 12, padding: '8px 12px', border: '1px solid #ddd', borderRadius: 6, resize: 'vertical' }}
      ...
      {/* Results */}
      {error && <div style={{ padding: '10px 14px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, color: '#b91c1c', fontSize: 13, wordBreak: 'break-all', marginBottom: 12 }}>{error}</div>}
      {result && (
        <pre style={{ background: '#f8fafc', border: '1px solid #e2e8f0', padding: 14, borderRadius: 8, maxHeight: 400, overflow: 'auto', fontSize: 12, lineHeight: 1.5 }}>
