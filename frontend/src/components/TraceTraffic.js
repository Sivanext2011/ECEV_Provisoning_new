    ...
    const fetchStatus = useCallback(async () => {
        try {
            ...
            if (res.ok) {
                ...
                if ((s.bam_fqdn || d.bam_fqdn) && !oamDomain)
                    setOamDomain(s.bam_fqdn || d.bam_fqdn);
                if ((s.bam_iam_url || d.iam_url) && !iamUrl)
                    setIamUrl(s.bam_iam_url || d.iam_url);
                if (s.bam_username && !loginUser)
                    setLoginUser(s.bam_username);
                if ((s.chf_fqdn || d.chf_fqdn) && !chfFqdn)
                    setChfFqdn(s.chf_fqdn || d.chf_fqdn);
                if (s.chf_port && !chfPort)
                    setChfPort(String(s.chf_port));
                if ((s.pcf_fqdn || d.pcf_fqdn) && !pcfFqdn)
                    setPcfFqdn(s.pcf_fqdn || d.pcf_fqdn);
