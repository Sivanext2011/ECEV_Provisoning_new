    ...
    try {
      if (pcAction === 'viewConsumers') {
        ...
        if (pcConsumerPO && pcConsumerCustExtId && pcConsumerContractExtId) {
          ...
          if (!r1.ok) throw new Error(`Consumer PO failed: ${(await r1.json()).detail || r1.status}`)
        }
        ...
        if (!r2.ok) throw new Error(`Provider update failed: ${(await r2.json()).detail || r2.status}`)
            ...
            <div style={{ border: '1px solid #fecaca', borderRadius: 8, padding: 16, background: '#fef2f2', marginBottom: 16 }}>
              ...
              {(pcAction === 'addConsumer' || pcAction === 'removeConsumer') && (
                <div style={{ display: 'grid', gap: 8, marginBottom: 12 }}>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: 11, fontWeight: 600, marginBottom: 2 }}>Consumer List External ID {pcAction === 'removeConsumer' ? '*' : `${pcConsumerListExtId ? '✓' : '(auto-generated)'}`}</label>
                    <input style={{ width: '100%', padding: '4px 8px', fontSize: 12, background: pcConsumerListExtId ? '#f0fff4' : '#fff' }} value={pcConsumerListExtId}
                      onChange={e => setPcConsumerListExtId(e.target.value)} placeholder="Auto-generated from MSISDN lookup" />
                  </div>
                </div>
              )}
